 ...
 * Author:   Mateusz Loskot, mateusz@loskot.net, Feilunzhou modified
 *
 ****************************************************************************/
...
bool OGRGeoJSONReader::AddFeature( VectorTile* poTile, OGRGeometry* poGeometry )
{
    ...
    {
        OGRFeature* poFeature = NULL;
        poFeature = new OGRFeature( poTile->getLayer()->GetLayerDefn() );
        poFeature->SetGeometryDirectly( poGeometry );

        bAdded = AddFeature( poTile, poFeature );
    }
...
OGRFeature* OGRGeoJSONReader::ReadFeature( VectorTile* poTile, json_object* poObj )
{
        ...
        json_object_get_type(poObjProps) == json_type_object )
    {
        ...
        json_object_object_foreachC( poObjProps, it )
        {
            ...
            else if( OFTInteger == eType )
            {
                ...
                if( EQUAL( it.key, poTile->getLayer()->GetFIDColumn() ) )
...
/* -------------------------------------------------------------------- */
    if( -1 == poFeature->GetFID() )
    {
        ...
        if( NULL != poObjId
            && EQUAL( OGRGeoJSONLayer::DefaultFIDColumn, poTile->getLayer()->GetFIDColumn() )
            && OFTInteger == GeoJSONPropertyToFieldType( poObjId ) )
        {
            poFeature->SetFID( json_object_get_int( poObjId ) );
            int nField = poFeature->GetFieldIndex( poTile->getLayer()->GetFIDColumn() );
            if( -1 != nField )
...
OGRGeoJSONReader::ReadFeatureCollection( VectorTile* poTile, json_object* poObj )
{
    ...
    if( json_type_array == json_object_get_type( poObjFeatures ) )
    {
        ...
        {
            poObjFeature = json_object_array_get_idx( poObjFeatures, i );
            poFeature = ReadFeature( poTile, poObjFeature );
            bAdded = AddFeature( poTile, poFeature );
            //CPLAssert( bAdded );
        }
